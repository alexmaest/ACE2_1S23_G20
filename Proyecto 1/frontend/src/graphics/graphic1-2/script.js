...
function resizeCanvas () {
 ...
 function SUMMIT () {
  ...
  const paramY = 'segundos(s)'
  const paramX = 'pomodoros(p)'

  const n2 = document.getElementById('num').value
...
class BarChart {
  ...
  drawLegend () { // dibuja los cuadros de color con su label respectivo, identificador de que cosa representa cada una de las barras
    ...
    let ulActual = this.ul

    // obtengo los valores de data
    const valores = Object.values(this.options.data)
    let contador = 0// contador para recorrer ese array de valores

    let contador2 = 1// contador para recorrer ese array de valores
