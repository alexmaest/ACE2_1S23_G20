...
GTitle.prototype.setTextAlignment = function (textAlignment) {
  if (
    textAlignment === this.parent.CENTER ||
    textAlignment === this.parent.LEFT ||
    textAlignment === this.parent.RIGHT
  ) {
...
function GAxisLabel(parent, type, dim) {
  ...
  // General properties
  this.type =
    type === this.parent.BOTTOM ||
    type === this.parent.TOP ||
    type === this.parent.LEFT ||
    type === this.parent.RIGHT
      ? type
      : this.parent.BOTTOM
  this.dim = dim.slice()
  this.relativePos = 0.5
...
GAxisLabel.prototype.setFontProperties = function (
  fontName,
  fontColor,
  fontSize
) {
...
GAxis.prototype.roundPlus = function (number, sigDigits) {
  ...
  number =
    Math.round(number * Math.pow(10, sigDigits)) / Math.pow(10, sigDigits)
...
GAxis.prototype.obtainLinearTicks = function () {
  ...
  if (this.ticksSeparation > 0) {
    step =
      this.lim[1] > this.lim[0] ? this.ticksSeparation : -this.ticksSeparation
    ...
    if (step === 0 || Math.abs(step) > Math.abs(this.lim[1] - this.lim[0])) {
      sigDigits++
      step = this.roundPlus(
        (this.lim[1] - this.lim[0]) / this.nTicks,
        sigDigits
      )
    }
  ...
  if (nSteps > 0) {
    ...
    var firstTick =
      this.lim[0] + (this.lim[1] - this.lim[0] - nSteps * step) / 2
...
GAxis.prototype.updateTicksInside = function () {
  ...
  if (this.type === this.parent.BOTTOM || this.type === this.parent.TOP) {
    for (i = 0; i < n; i++) {
      this.ticksInside[i] =
        this.plotTicks[i] >= 0 && this.plotTicks[i] <= this.dim[0]
    }
  } else {
    for (i = 0; i < n; i++) {
      this.ticksInside[i] =
        -this.plotTicks[i] >= 0 && -this.plotTicks[i] <= this.dim[1]
    }
  }
}
  ...
  if (this.log) {
        ...
        if (isExactLogValue) {
          ...
          if (this.expTickLabels) {
            ...
            if (logValue > -3.1 && logValue < 3.1) {
              this.tickLabels[i] =
                logValue >= 0 ? '' + Math.round(tick) : '' + tick
            } else {
              this.tickLabels[i] = '1e' + logValue
            }
          }
    ...
    for (i = 0; i < n; i++) {
      ...
      if (tick % 1 === 0) {
        ...
        typeof this.ticksPrecision !== 'undefined' &&
        this.ticksPrecision >= 0
      ) {
      ...
      if (!this.fixedTicks) {
        ...
        if (this.log) {
          ...
          if (this.ticksSeparation > 0) {
            ...
            if (
              step === 0 ||
              Math.abs(step) > Math.abs(this.lim[1] - this.lim[0])
            ) {
              sigDigits++
              step =
                n === 1
...
GAxis.prototype.setAllFontProperties = function (
  fontName,
  fontColor,
  fontSize
) {
...
function GHistogram(parent, type, dim, plotPoints) {
  ...
  // General properties
  this.type =
    type === GPlot.VERTICAL || type === GPlot.HORIZONTAL ? type : GPlot.VERTICAL
  this.dim = dim.slice()
  this.plotPoints = []
    ...
    for (var i = 0; i < nPoints; i++) {
      ...
      if (point.isValid()) {
        ...
        if (Math.abs(x2 - x1) > 2 * lw && Math.abs(y2 - y1) > 2 * lw) {
          ...
          (this.type === GPlot.VERTICAL &&
            x2 !== x1 &&
            !(y1 === y2 && (y1 === 0 || y1 === -this.dim[1]))) ||
          (this.type === GPlot.HORIZONTAL &&
            y2 !== y1 &&
            !(x1 === x2 && (x1 === 0 || x1 === this.dim[0])))
        ) {
          this.parent.rect(x1, y1, x2, y2)
...
GHistogram.prototype.setType = function (type) {
  if (
    type !== this.type &&
    (type === GPlot.VERTICAL || type === GPlot.HORIZONTAL)
  ) {
    this.type = type
    this.updateArrays()
  }
...
GHistogram.prototype.setFontProperties = function (
  fontName,
  fontColor,
  fontSize
) {
...
GLayer.prototype.valueToXPlot = function (x) {
  if (this.xLog) {
    return (
      (this.dim[0] * Math.log(x / this.xLim[0])) /
...
GLayer.prototype.valueToYPlot = function (y) {
  if (this.yLog) {
    return (
      (-this.dim[1] * Math.log(y / this.yLim[0])) /
  ...
  if (this.xLog) {
    return Math.exp(
      Math.log(this.xLim[0]) +
        (Math.log(this.xLim[1] / this.xLim[0]) * xPlot) / this.dim[0]
    )
  } else {
  ...
  if (this.yLog) {
    return Math.exp(
      Math.log(this.yLim[0]) -
        (Math.log(this.yLim[1] / this.yLim[0]) * yPlot) / this.dim[1]
    )
  } else {
...
GLayer.prototype.isInside = function () {
  ...
  return valid
    ? xPlot >= 0 && xPlot <= this.dim[0] && -yPlot >= 0 && -yPlot <= this.dim[1]
    : false
}
  ...
  if (this.isInside(xPlot, yPlot)) {
    ...
    for (var i = 0; i < nPoints; i++) {
      if (this.inside[i]) {
        point = this.plotPoints[i]
        distSq =
          Math.pow(point.getX() - xPlot, 2) + Math.pow(point.getY() - yPlot, 2)

        if (
          distSq < Math.max(Math.pow(this.pointSizes[i % nSizes] / 2.0, 2), 25)
        ) {
          if (distSq < minDistSq) {
            minDistSq = distSq
            pointIndex = i
...
GLayer.prototype.obtainBoxIntersections = function (plotPoint1, plotPoint2) {
  ...
  if (plotPoint1.isValid() && plotPoint2.isValid()) {
    ...
    var dontCut =
      (inside1 && inside2) ||
      (x1 < 0 && x2 < 0) ||
      (x1 > this.dim[0] && x2 > this.dim[0]) ||
      (-y1 < 0 && -y2 < 0) ||
      (-y1 > this.dim[1] && -y2 > this.dim[1])
    ...
    if (!dontCut) {
      ...
      if (inside1 || inside2) {
          ...
          if (nCuts > 1) {
            ...
            if (nCuts > 1) {
              ...
              if (nCuts > 1) {
                ...
                if (nCuts > 1) {
                  nCuts = this.removePointFromCuts(
                    this.cuts,
                    nCuts,
                    pointInside,
                    0.001
        ...
        if (nCuts === 2) {
          if (
            (Math.pow(this.cuts[0][0] - x1, 2) + Math.pow(this.cuts[0][1] - y1),
            2) >
            Math.pow(this.cuts[1][0] - x1, 2) +
              Math.pow(this.cuts[1][1] - y1, 2)
          ) {
      ...
      if ((inside1 || inside2) && nCuts !== 1) {
        ...
        console.log(
          'There should be either 0 or 2 cuts!!! ' + nCuts + ' were found'
        )
      }
    }
  }
...
GLayer.prototype.removeDuplicatedCuts = function (cuts, nCuts, tolerance) {
  ...
  for (var i = 0; i < nCuts; i++) {
    ...
    for (var j = 0; j < counter; j++) {
      if (
        Math.abs(cuts[j][0] - cuts[i][0]) <= tolerance &&
...
GLayer.prototype.removePointFromCuts = function (
  cuts,
  nCuts,
  plotPoint,
  tolerance
  ...
  if (arguments.length === 0) {
    ...
    if (nColors === 1 && nSizes === 1) {
      ...
      for (i = 0; i < nPoints; i++) {
        if (this.inside[i]) {
          this.parent.ellipse(
            this.plotPoints[i].getX(),
            this.plotPoints[i].getY(),
            this.pointSizes[0],
            this.pointSizes[0]
          )
        }
  ...
  for (var i = 0; i < nPoints - 1; i++) {
    if (this.inside[i] && this.inside[i + 1]) {
      this.parent.line(
        this.plotPoints[i].getX(),
        this.plotPoints[i].getY(),
        this.plotPoints[i + 1].getX(),
        this.plotPoints[i + 1].getY()
      )
    } else if (
      this.plotPoints[i].isValid() &&
      this.plotPoints[i + 1].isValid()
    ) {
      // At least one of the points is outside the inner region.
      // Obtain the valid line box intersections
      var nCuts = this.obtainBoxIntersections(
        this.plotPoints[i],
        this.plotPoints[i + 1]
      )
...
GLayer.prototype.drawLine = function () {
  ...
  if (typeof slope !== 'undefined') {
    if (this.xLog && this.yLog) {
      point1 = new GPoint(
        this.xLim[0],
        Math.pow(10, (slope * Math.log(this.xLim[0])) / Math.LN10 + yCut)
      )
      point2 = new GPoint(
        this.xLim[1],
        Math.pow(10, (slope * Math.log(this.xLim[1])) / Math.LN10 + yCut)
      )
    } else if (this.xLog) {
      point1 = new GPoint(
        this.xLim[0],
        (slope * Math.log(this.xLim[0])) / Math.LN10 + yCut
      )
      point2 = new GPoint(
        this.xLim[1],
        (slope * Math.log(this.xLim[1])) / Math.LN10 + yCut
      )
    } else if (this.yLog) {
      point1 = new GPoint(
        this.xLim[0],
        Math.pow(10, slope * this.xLim[0] + yCut)
      )
      point2 = new GPoint(
        this.xLim[1],
        Math.pow(10, slope * this.xLim[1] + yCut)
      )
    } else {
...
GLayer.prototype.drawLabel = function (point) {
  ...
  if (this.isValidNumber(xPlot) && this.isValidNumber(yPlot)) {
    ...
    this.parent.rect(
      xLabelPos - delta,
      yLabelPos - this.fontSize - delta,
      this.parent.textWidth(point.getLabel()) + 2 * delta,
      this.fontSize + 2 * delta
    )
...
GLayer.prototype.drawPolygon = function (polygonPoints, polygonColor) {
  ...
  if (polygonPoints.length > 2) {
    ...
    for (i = 0; i < nPoints; i++) {
      ...
      var cond =
        (label === 'single cut' && nextLabel === 'single cut') ||
...
GLayer.prototype.drawAnnotation = function (text, x, y, horAlign, verAlign) {
  ...
  if (
    this.isValidNumber(xPlot) &&
    this.isValidNumber(yPlot) &&
    this.isInside(xPlot, yPlot)
  ) {
    if (
      horAlign !== this.parent.CENTER &&
      horAlign !== this.parent.RIGHT &&
...
GLayer.prototype.setXYLim = function () {
  ...
  if (
    xMin !== xMax &&
    yMin !== yMax &&
    this.isValidNumber(xMin) &&
    this.isValidNumber(xMax) &&
    this.isValidNumber(yMin) &&
    this.isValidNumber(yMax)
  ) {
...
GLayer.prototype.setPoint = function () {
  ...
  if (index < nPoints) {
    ...
    this.plotPoints[index] = new GPoint(
      this.valueToXPlot(x),
      this.valueToYPlot(y),
      label
    )
...
GLayer.prototype.addPoint = function () {
  ...
  this.plotPoints.push(
    new GPoint(this.valueToXPlot(x), this.valueToYPlot(y), label)
  )
...
GLayer.prototype.addPoints = function (newPoints) {
  ...
  for (var i = 0; i < nNewPoints; i++) {
    ...
    this.plotPoints.push(
      new GPoint(
        this.valueToXPlot(newPoint.getX()),
        this.valueToYPlot(newPoint.getY()),
        newPoint.getLabel()
      )
    )
    this.inside.push(this.isInside(this.plotPoints[this.plotPoints.length - 1]))
...
GLayer.prototype.setAllFontProperties = function (
  fontName,
  fontColor,
  fontSize
) {
...
GPlot.prototype.getScreenPosAtValue = function (xValue, yValue) {
  var xScreen =
    this.mainLayer.valueToXPlot(xValue) + (this.pos[0] + this.mar[1])
  var yScreen =
    this.mainLayer.valueToYPlot(yValue) +
...
GPlot.prototype.removePointAt = function () {
  ...
  var plotPos = this.getPlotPosAt(xScreen, yScreen)
  var pointIndex = this.getLayer(layerId).getPointIndexAtPlotPos(
    plotPos[0],
    plotPos[1]
  )
...
GPlot.prototype.isOverPlot = function () {
  ...
  return (
    xScreen >= this.pos[0] &&
    xScreen <= this.pos[0] + this.outerDim[0] &&
    yScreen >= this.pos[1] &&
    yScreen <= this.pos[1] + this.outerDim[1]
  )
}
...
GPlot.prototype.isOverBox = function () {
  ...
  return (
    xScreen >= this.pos[0] + this.mar[1] &&
    xScreen <= this.pos[0] + this.outerDim[0] - this.mar[3] &&
    yScreen >= this.pos[1] + this.mar[2] &&
    yScreen <= this.pos[1] + this.outerDim[1] - this.mar[0]
  )
}
...
GPlot.prototype.moveHorizontalAxesLim = function (delta) {
  ...
  if (this.xLog) {
    deltaLim = Math.exp(
      (Math.log(this.xLim[1] / this.xLim[0]) * delta) / this.dim[0]
    )
...
GPlot.prototype.moveVerticalAxesLim = function (delta) {
  ...
  if (this.yLog) {
    deltaLim = Math.exp(
      (Math.log(this.yLim[1] / this.yLim[0]) * delta) / this.dim[1]
    )
...
GPlot.prototype.zoom = function () {
  ...
  if (arguments.length === 3) {
    ...
    if (this.xLog) {
      deltaLim = Math.exp(Math.log(this.xLim[1] / this.xLim[0]) / (2 * factor))
      offset = Math.exp(
        (Math.log(this.xLim[1] / this.xLim[0]) / factor) *
          (0.5 - plotPos[0] / this.dim[0])
      )
    ...
    if (this.yLog) {
      deltaLim = Math.exp(Math.log(this.yLim[1] / this.yLim[0]) / (2 * factor))
      offset = Math.exp(
        (Math.log(this.yLim[1] / this.yLim[0]) / factor) *
          (0.5 + plotPos[1] / this.dim[1])
      )
    ...
    factor = arguments[0]
    var centerValue = this.mainLayer.plotToValue(
      this.dim[0] / 2,
      -this.dim[1] / 2
    )
...
GPlot.prototype.shiftPlotPos = function (valuePlotPos, newPlotPos) {
  ...
  if (this.xLog) {
    deltaLim = Math.exp(
      (Math.log(this.xLim[1] / this.xLim[0]) * deltaXPlot) / this.dim[0]
    )
  ...
  if (this.yLog) {
    deltaLim = Math.exp(
      (-Math.log(this.yLim[1] / this.yLim[0]) * deltaYPlot) / this.dim[1]
    )
...
GPlot.prototype.drawLegend = function (text, xRelativePos, yRelativePos) {
  ...
  for (var i = 0; i < text.length; i++) {
    var plotPosition = [
      xRelativePos[i] * this.dim[0],
      -yRelativePos[i] * this.dim[1],
    ]
    var position = this.mainLayer.plotToValue(
      plotPosition[0] + rectSize,
      plotPosition[1]
    )
    ...
    if (i === 0) {
      ...
      this.parent.rect(plotPosition[0], plotPosition[1], rectSize, rectSize)
      this.layerList[i - i].drawAnnotation(
        text[i],
        position[0],
        position[1],
        this.parent.LEFT,
...
GPlot.prototype.setAllFontProperties = function (
  fontName,
  fontColor,
  fontSize
) {
...
GPlot.prototype.activateZooming = function () {
  ...
  if (
    increaseButton === this.parent.LEFT ||
    increaseButton === this.parent.RIGHT ||
    increaseButton === this.parent.CENTER
  ) {
    this.increaseZoomButton = increaseButton
  }

  if (
    decreaseButton === this.parent.LEFT ||
    decreaseButton === this.parent.RIGHT ||
    decreaseButton === this.parent.CENTER
  ) {
    this.decreaseZoomButton = decreaseButton
  }

  if (
    increaseKeyModifier === this.parent.SHIFT ||
    increaseKeyModifier === this.parent.CONTROL ||
    increaseKeyModifier === this.parent.ALT ||
    increaseKeyModifier === GPlot.NONE
  ) {
  ...
  if (
    decreaseKeyModifier === this.parent.SHIFT ||
    decreaseKeyModifier === this.parent.CONTROL ||
    decreaseKeyModifier === this.parent.ALT ||
    decreaseKeyModifier === GPlot.NONE
  ) {
...
GPlot.prototype.activateCentering = function () {
  ...
  if (
    keyModifier === this.parent.SHIFT ||
    keyModifier === this.parent.CONTROL ||
    keyModifier === this.parent.ALT ||
    keyModifier === GPlot.NONE
  ) {
...
GPlot.prototype.saveResetLimits = function () {
  if (
    typeof this.xLimReset === 'undefined' ||
    typeof this.yLimReset === 'undefined'
  ) {
...
GPlot.prototype.clickEvent = function (event) {
  ...
  if (this.isOverBox()) {
    ...
    if (this.zoomingIsActive) {
      if (
        button === this.increaseZoomButton &&
        modifier === this.increaseZoomKeyModifier
      ) {
        ...
        button === this.decreaseZoomButton &&
        modifier === this.decreaseZoomKeyModifier
      ) {
        // Save the axes limits
        if (this.resetIsActive) {
          this.saveResetLimits()
    ...
    if (
      ...
      button === this.centeringButton &&
      modifier === this.centeringKeyModifier
    ) {
      // Save the axes limits
      if (this.resetIsActive) {
        this.saveResetLimits()
    ...
    if (
      ...
      modifier === this.resetKeyModifier
    ) {
      if (
        typeof this.xLimReset !== 'undefined' &&
        typeof this.yLimReset !== 'undefined'
...
GPlot.prototype.mouseDownEvent = function (event) {
  ...
  if (this.isOverBox()) {
    ...
    if (
      ...
      button === this.panningButton &&
      modifier === this.panningKeyModifier
    ) {
      addListeners = true
    ...
    if (
      ...
      button === this.labelingButton &&
      modifier === this.labelingKeyModifier
    ) {
      addListeners = true
...
GPlot.prototype.touchMoveEvent = function (event) {
  ...
  if (
    ...
    this.zoom(
      distance / this.zoomStartDistance,
